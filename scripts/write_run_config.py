...
CONTENT = r'''"""Load and validate operator Run configuration (run.yaml / run.json)."""
...
def main() -> None:
    path = Path(__file__).resolve().parents[1] / "src" / "fgo_auto" / "run" / "run_config.py"
    path.write_text(CONTENT, encoding="utf-8", newline="\n")
    data = path.read_bytes()
    assert b"\x00" not in data, "UTF-16 corruption detected"
