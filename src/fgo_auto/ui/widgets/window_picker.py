...
class WindowPickerFrame(ctk.CTkFrame):
    def __init__(self, master, on_select, **kwargs) -> None:
        ...
        ctk.CTkLabel(self, text="視窗綁定（Window pick）").pack(anchor="w", padx=8, pady=(8, 4))
        row = ctk.CTkFrame(self)
        row.pack(fill="x", padx=8, pady=4)
        self._rule_entry = ctk.CTkEntry(row, placeholder_text="window_title_rule")
        ...
        self._list = ctk.CTkOptionMenu(self, values=["（先重新整理）"], command=self._picked)
    ...
    def refresh(self) -> None:
        ...
        if not rule:
            self._status.configure(text="請輸入 window_title_rule")
            return
        try:
        ...
        if not self._candidates:
            ...
            self._list.configure(values=["（無符合）"])
            return
        labels = [
            f"{w.handle} | {w.width}x{w.height} | {w.title[:60]}"
            for w in self._candidates
        ]
        self._list.configure(values=labels)
        self._list.set(labels[0])
        self._picked(labels[0])
        self._status.configure(text=f"找到 {len(self._candidates)} 個候選")
